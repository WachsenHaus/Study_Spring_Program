...
<div class="container">
	...
	<form name="myForm" action="insert.jsp" method="post" novalidate>
		...
		<button class="btn btn-outline-primary" type="submit" ng-disabled="myForm.id.$invalid || myForm.nick.$invalid">제출</button>
	</form>
	...
	<p> 닉네임가 청결(순결) 한지 여부 : <strong>{{myForm.nick.$pristine}}</strong></p>
	<p> 닉네임가 더렵혀졌는지 여부: <strong>{{myForm.nick.$dirty}}</strong>
	
	<p>닉네임 입력    했는지 여부 : <strong>{{myForm.nick.$error.required}}</strong></p>
	<p>닉네임 입력 안했는지 여부 : <strong>{{!myForm.nick.$error.required}}</strong></p>
	<p>닉네임을 최소 글자이상 입력 했는지 여부 <strong>{{!myForm.nick.$error.minlength}}</strong></p>
	<p>닉네임을 최대 글자이하로 입력 했는지 여부 <strong>{{myForm.nick.$error.maxlength}}</strong></p>
	<p>닉네임을 정규표현식에 맞게 입력 했는지 여부 <strong>{{myForm.nick.$error.pattern}}</strong></p>
	<p>닉네임을 정규표현식에 맞게 입력 안했는지 여부 <strong>{{!myForm.nick.$error.pattern}}</strong></p>
	
	<p>폼 전체의 유효성 여부 <strong>{{myForm.$valid}}</strong></p>
	<p>폼 전체가 유효하지않은지 여부 <strong>{{myForm.$invalid}}</strong></p>
